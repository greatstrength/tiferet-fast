...
### 5.5 Publishing documentation — not currently a behavior of this domain
*Whether FastAPI's own native OpenAPI generation is reconciled with tiferet-openapi's `generate_spec`/`get_docs_spec`.*

It is not. `build_fast_app` constructs `FastAPIApp(title=f'{interface_id} API', middleware=middleware)` with no `openapi_url` override and no route wired to `get_docs_spec` or the deprecated `create_docs_handler` alias. `grep` for `create_docs_handler`, `get_docs_spec`, `app.openapi`, and `openapi_url` across `tiferet_fast/` returns nothing. FastAPI's native `/docs` and `/openapi.json` work today only because `build_router` already passes `summary`/`description`/`tags`/`response_model` to `add_api_route`, which is enough for FastAPI to derive its own schema independently — the two schema sources are not "in parallel and unreconciled" so much as tiferet-fast never invokes tiferet-openapi's Generate/Publish steps (`docs/core-domain-distillation.md` Section 5.3/5.4 in the `tiferet-openapi` companion) at all.
...
- **No wiring exists between FastAPI's native schema generation and `generate_spec`/`get_docs_spec`.** Not previously named as a gap because tiferet-fast's `AGENTS.md`/`README.md` describe `response_model` resolution as sufficient for "native Swagger schema generation" without ever mentioning `generate_spec`/`get_docs_spec` at all — so this is better read as an undecided design question than a regression (Section 5.5).
...
- Generating an OpenAPI 3.0 specification and serving a documentation page from it — `OpenApiSessionContext.generate_spec`/`get_docs_spec` exist for this in `tiferet-openapi`; no route in `tiferet-fast` currently calls either (Section 5.5).
...
4. **Decide the Publish story.** No RFP should assume tiferet-fast should wire a docs route to `get_docs_spec` without a decision first — the current "FastAPI's native generation is authoritative" shape may be intentional (Section 5.5).
