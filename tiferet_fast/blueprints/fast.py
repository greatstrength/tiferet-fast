...
def create_fast_request_handler(header_keys: dict) -> Callable:
    ...
    Wraps ``create_openapi_request_context`` so Serve sees generated request
    and correlation ids without ``FastApiContext`` importing starlette_context.
    ...
    def handler(interface_id: str,
        ...
        return create_openapi_request_context(
            interface_id,
            feature_id,
            headers=merged,
            data=data,
...
def build_fast_session_context(app_session: AppSession,
    ...
    :param create_request_handler: Optional request-construction handler;
        defaults to create_fast_request_handler with library header names
        when omitted. An explicit handler is not wrapped again.
    ...
    return core.compose_session_context(
        ...
        app_container,
        resolver,
        create_request_handler=create_request_handler or create_fast_request_handler(
            parse_context_header_options({}),
        ),
